                                                                            ...
                                                                            <div
                                                                                ...
                                                                                <div
                                                                                    ...
                                                                                    <p
                                                                                        ...
                                                                                        // Get the product - use get_product() to get actual product (including variation if applicable)
                                                                                        $email_product = $item->get_product();
                                                                                        $book_file_displayed = false; // Flag to avoid duplicate book file displays
                                                                                       
                                                                                        // Only proceed if ACF is available and product exists
                                                                                        if ( function_exists( 'get_field' ) && is_object( $email_product ) ) {
                                                                                            $actual_product_id = $email_product->get_id();
                                                                                            $parent_product_id = $item->get_product_id();
                                                                                            // First, check the actual purchased product for 'book_file' (variation or simple product)
                                                                                            $book_file = get_field( 'book_file', $actual_product_id );

                                                                                            // If not found on variation, check the parent product
                                                                                            if ( ! $book_file || empty( $book_file['url'] ) ) {
                                                                                                if ( $parent_product_id !== $actual_product_id ) {
                                                                                                    $book_file = get_field( 'book_file', $parent_product_id );
                                                                                                } else {
                                                                                                }
                                                                                            }

                                                                                            // If book_file found on this product, display it
                                                                                            if ( $book_file && ! empty( $book_file['url'] ) && ! $book_file_displayed ) {
                                                                                                echo '<p><a href="' . esc_url( $book_file['url'] ) . '" target="_blank">' . esc_html__('Download book', 'nutrisslim-suite') . '</a></p>';
                                                                                                $book_file_displayed = true;
                                                                                            } else {
                                                                                            }

                                                                                            // If not found and it's a bundle, also check child products
                                                                                            $parent_product_for_bundle = wc_get_product( $parent_product_id );

                                                                                            if ( ! $book_file_displayed && is_object( $parent_product_for_bundle ) && $parent_product_for_bundle->get_type() === 'nutrisslim' ) {
                                                                                                foreach ( $order->get_items() as $child_item_id => $child_item ) {
                                                                                                    ...
                                                                                                    $child_parent_id = $child_item->get_meta( 'nutrisslim_parent_id', true );

                                                                                                    if ( $child_parent_id && $child_parent_id == $parent_product_id ) {
