...
function add_to_cart_link() {
    echo '<div id="foocart"><a class="uwcc-open-cart-546612" href="#">Aggiungi al carrello</a></div>';
}
...
function custom_get_additional_fees_settings() {
    ...
    $additional_fees_settings = array(
        ...
        'priority_delivery' => array(
            'name' => __('Consegna prioritaria', 'nutrisslim-suite'),
            'type' => 'number',
        ...
        'package_insurance' => array(
            'name' => __('Assicurazione sulla spedizione', 'nutrisslim-suite'),
            'type' => 'number',
...
function add_custom_fees_to_cart() {
    ...
    $custom_fees = [
        ...
        'rf_zavarovanje_narocila' => [
            'label' => __('Assicurazione sulla spedizione', 'nutrisslim-suite'),
        ...
        'rf_priority_delivery' => [
            'label' => __('Consegna prioritaria', 'nutrisslim-suite'),
...
<div id="delivery-option" class="rf-custom-option">
    <h3><?php echo __('Scegli una consegna rapida e sicura', 'nutrisslim-suite'); ?></h3>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_priority_delivery_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_priority_delivery">
                <div class="rf-option-title"><?php echo __('Salta la riga', 'nutrisslim-suite'); ?>
                    (+<?php echo wc_price($priority_delivery); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Scegli l\'imballaggio prioritario per la spedizione più veloce. Selezionando questa opzione, il tuo ordine verrà elaborato prima degli altri.', 'nutrisslim-suite'); ?>
                </div>
            </label>
        </div>
    ...
    <div class="delivery-option rf-option-container" style="<?php echo $is_checked_zavarovanje_bg; ?>">
        <div class="rf-option-left">
            ...
            <label for="rf_zavarovanje_narocila">
                <div class="rf-option-title"><?php echo __('Assicurazione sulla spedizione', 'nutrisslim-suite'); ?>
                    (+<?php echo wc_price($package_insurance); ?>)</div>
                <div class="rf-option-description">
                    <?php echo __('Proteggi la consegna del pacco e garantisci una soluzione gratuita in caso di prodotti danneggiati o mancanti. Invieremo un nuovo pacco senza costi aggiuntivi.', 'nutrisslim-suite'); ?>
                </div>
            </label>
        </div>
...
<div class="rf-custom-option select-shipping">
    <h3><?php echo __('Consegna', 'woocommmerce'); ?></h3>
...
function custom_woocommerce_loop_add_to_cart_link($html, $product) {
    ...
    $custom_text = __( 'Aggiungi al carrello', 'woocommerce' );
...
function cart_upsell() {
    ...
    if (!empty($items)) {
        ...
        $output .= '<h3>' . __('Offerta speciale solo qui!', 'nutrisslim-suite') . '</h3>';
        $output .= '<p>' . __('Aggiungi per ordinare e risparmiare!', 'nutrisslim-suite') . '</p>';
        ...
        foreach ($items as $item) {
            ...
            $output .= '<button class="cart-button_swiper add-to-cart-icon" data-offer="cart_upsell" data-product-id="' . $pid . '" data-quantity="1" data-price="' . $offer_price . '" ' . $disabled . '>' . __('Aggiungi al carrello', 'woocommerce') . '</button>';
...
function add_meta_to_order_fees($item, $fee_key, $fee, $order) {
    ...
    $fee_sifrant_map = array(
        ...
        'assicurazione-sulla-spedizione' => get_option('custom_package_insurance_sifrant'),
        'supplemento-per-il-pagamento-in-contrassegno' => 'STRL',
    );
...
//Add new column in admin product list
...
function add_stock_status_column($columns) {
    ...
    foreach ($columns as $key => $value) {
        ...
        // Add the new column after the 'sku' column
        if ($key === 'sku') {
            $new_columns['stock_status'] = __('Stock Status', 'woocommerce');
        }
    }
...
// Populate the new column with stock status
// add_action('manage_product_posts_custom_column', 'show_stock_status_column', 10, 2);
function show_stock_status_column($column, $postid) {
    if ($column === 'stock_status') {
        $product = wc_get_product($postid);
        if ($product) {
            $stock_status = $product->get_stock_status();
            if ($stock_status === 'instock') {
                echo '<span style="background-color: #d4edda; color: #155724; padding: 3px 8px; border-radius: 3px; display: inline-block;">' . __('Na zalogi', 'woocommerce') . '</span>';
            } elseif ($stock_status === 'outofstock') {
                echo '<span style="background-color: #f8d7da; color: #721c24; padding: 3px 8px; border-radius: 3px; display: inline-block;">' . __('Ni na zalogi', 'woocommerce') . '</span>';
            } else {
                echo '<span style="background-color: #e2e3e5; color: #383d41; padding: 3px 8px; border-radius: 3px; display: inline-block;">' . __('Unknown', 'woocommerce') . '</span>';
            }
        }
    }
}
