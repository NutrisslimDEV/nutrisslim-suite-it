...
<?php
    ...
    if ($product_type == 'nutrisslim') {
        // $singleLabel = ' paket';
        $singleLabel = __('pacco', 'nutrisslim-suite');
        $dvaLabel = 'pacchi';
        // $triLabel = ' paketi';
        $triLabel = __('pacchi', 'nutrisslim-suite');
    } else {
        // $singleLabel = ' kom';
        $singleLabel = __('pezzi', 'nutrisslim-suite');
        $dvaLabel = 'kom'; 
        $triLabel = 'kom';                    
        // $triLabel = ' kom';                    
        $triLabel = __('pezzo', 'nutrisslim-suite');
    }
...
<div class="customAddToCart">
    <div class="topInfo">
        ...
        <span class="subscriptiolLabel" style="display:none;">
            <?php printf(__('per <span class="numdays">%s</span> giorni', 'nutrisslim-suite'), $subscription_periods[0]); ?></span>
    </div>
    <p><?php echo __('Seleziona la quantità di prodotti:', 'nutrisslim-suite'); ?></p>
        ...
        <form>
            <div class="qSelect grid-3">
                ...
                <label class="col top">
                    ...
                    <div class="paket">
                        ...
                        <div class="choice">
                            <?php echo __('La scelta migliore', 'nutrisslim-suite'); ?>
                        </div>
                    </div>
                </label>
                <?php        
            ...
            <div class="tabsHolder">
                <div class="tabsItems grid-2">
                    <div class="col tab subscr">
                        <?php echo __('Iscriviti e risparmia', 'nutrisslim-suite'); ?><br />
                        <span class="warning">-<?php echo $discount_one_subscription; ?>%
                            <?php echo __('SCONTO', 'nutrisslim-suite'); ?><br /></span>
                    </div>
                    <div class="col tab active onetime"><?php echo __('Acquisto unico', 'nutrisslim-suite'); ?>
                    </div>
                </div>
                <div class="contWrap">
                    <div class="onetimeContent">
                        <p>
                            <strong>
                                <?php 
                                    printf( 
                                        __( 'Passa all\'abbonamento e risparmia %s', 'nutrisslim-suite' ), 
                            ...
                            <?php 
                                printf( 
                                    __( 'Puoi avere lo stesso per soli %s. Un modo conveniente per ottenere i migliori prezzi. L\'abbonamento può essere modificato o annullato in qualsiasi momento!', 'nutrisslim-suite' ), 
                    ...
                    <div style="display:none;" class="subscrContent">
                        <p><?php echo __('Seleziona il periodo di abbonamento:', 'nutrisslim-suite'); ?></p>
                        <select name="subscription" class="subscription">
                            <?php
                                    foreach ($subscription_periods as $subscription_period) {
                                        echo '<option value="' . $subscription_period . '">' . __('Spedisci ogni', 'nutrisslim-suite') . ' ' . $subscription_period . ' ' . __('giorni', 'nutrisslim-suite') . '</option>';
                                    }
                                ?>
                        </select>
                        <div class="grid info">
                            <div class="col-6 cancel">
                                <?php echo __('L\'abbonamento può essere annullato in qualsiasi momento.', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-6 lowest">
                                <?php echo __('Prezzo più basso verificato', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-12 learn"><a class="moreabout"
                                    href="#"><?php echo __('Scopri di più sull\'abbonamento', 'nutrisslim-suite') ?></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <?php
                    if ( !$product->is_in_stock() ) {
                        ...
                        echo '<p style="text-align:center"><span class="out-of-stock">' . __( 'Esaurito', 'woocommerce' ) . '</span></p>';
                    } else {
                ?>
            <button class="elementor-button add-to-cart-icon" data-product-id="<?php echo $product->get_id(); ?>"
                data-quantity="1" type="submit"><?php echo __('Aggiungi al carrello', 'woocommerce'); ?> | <span
                    class="buttonPrice"><?php echo wc_price($sale_price); ?></span> <span class="buttonTime"
                    style="display:none;"><?php printf(__('ogni <span class="numdays">%s</span> giorni', 'nutrisslim-suite'), $subscription_periods[0]) ?></span></button>
            <?php
            ...
            <p <?php echo $vis; ?>class="prihranek">
                <?php printf(__('Salva <span class="prihrana">%s</span>', 'nutrisslim-suite'), wc_price($regular_price - $sale_price)); ?>
            </p>
            ...
            <?php
                    if ( $product->is_in_stock() ) {
                        // If the product is out of stock, display "Out of Stock" text
                        echo '<p class="onstock">' . __( 'In magazzino, pronto per la consegna', 'nutrisslim-suite' ) . '</p>';
                    }                
                ?>
    ...
    <div class="featureWrapper">
        <div class="feature return"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_vracilo_ico.svg" />
            <?php printf(__('%s-giorni di garanzia soddisfatti o rimborsati', 'nutrisslim-suite'), 60); ?></div>
        <div class="feature free"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_free_delivery_ico.svg" />
            <span><?php printf(__('Spedizione gratuita per ordini superiori a %s', 'nutrisslim-suite'), wc_price(get_free_shipping_threshold())); ?></span>
        </div>
        <div class="feature delivery"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_dostava_ico.svg" />
            <?php printf(__('Consegna entro 1-3 giorni lavorativi', 'nutrisslim-suite'), '1-2'); ?></div>
    </div>
    ...
    <p class="phone">
        <?php printf(__('Ordini e richieste possono essere effettuati al numero <strong>%s</strong>', 'nutrisslim-suite'), get_option('woocommerce_support_phone')); ?>
    </p>
</div>
<?php
    }
