...
function trusted_get_reviews_block_a() {
    ...
    echo '<h2 class="trusted-review-title">Recensioni dei clienti</h2>';
    ...
    echo '<div class="trusted-review-text">Incredibile</div>';
    ...
    foreach ($comments as $comment) {
        $rating = intval(get_comment_meta($comment->comment_ID, 'rating', true));
        $comment_content = !empty(trim($comment->comment_content)) ? esc_html($comment->comment_content) : esc_html__('Il cliente ha lasciato solo una valutazione.', 'woocommerce');
    ...
    */
    ...
    foreach ($comments as $comment) {
        ...
        echo '<div class="trusted-review-date">' . sprintf( 
            esc_html__( '%s fa da %s', 'nutrisslim-suite' ), 
            esc_html( $time_diff ), 
            htmlspecialchars( $comment->comment_author, ENT_QUOTES, 'UTF-8' ) 
        ) . '</div>';
        echo '<div class="trusted-revComment">';
        echo '<p class="threerows">' . $comment_content . '</p><p class="rmlink"><a href="#">' . esc_html__( 'Leggi di più', 'woocommerce' ) . '</a></p>';
        echo '</div>';
        echo '</div>'; // Close trusted-revSlid
        echo '</div>'; // Close swiper-slide
    }
    ...
    if ($best_review) {
        ...
        echo '<p>' . esc_html__('Da', 'woocommerce') . ' ' . esc_html($best_review->comment_author) . '</p>';
