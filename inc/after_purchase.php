    ...
    <div class="container">
        <div class="grid">
            <div id="orderInfo" class="col-8_sm-12">
                <h4><?php echo __('Visualizzazione degli ordini', 'woocommerce'); ?></h4>

                <div class="grid articles">
                    <div class="col-6 label"><?php echo __('Nome del prodotto', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Preventivo', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Quantità', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Totale', 'woocommerce'); ?></div>
                    <?php 
                        $totalproducts = 0;
                ...
                <div class="grid summary">
                    ...
                    <div class="col-8 text-right label"><?php echo __('Subtotale', 'woocommerce'); ?></div>
                    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
                    <div class="col-8 text-right label">
                        <?php echo __('Spedizione', 'woocommerce') . '<span class="wo-tax"> (senza IVA)</span>'; ?></div>
                    ...
                    <?php
                            foreach ( $order->get_fees() as $fee ) {
                                ...
                                echo '<div class="col-8 text-right label">' . __(esc_html( $fee_name ), 'nutrisslim-suite') . '<span class="wo-tax"> (senza IVA)</span>' .'</div>';
                                echo '<div class="col-4 text-right">' . $fee_total_incl_tax . '</div>';
                            }
                        ?>

                    <div class="col-8 text-right label znesek"><?php echo __('Totale', 'woocommerce'); ?></div>
                    <div class="col-4 text-right"><?php echo wp_kses_post($order_total); ?></div>
                    <div class="col-8 text-right"><?php echo __('(con IVA)', 'woocommerce'); ?>
                    </div>
            ...
            <div id="customerInfo" class="col-4_sm-12">
                <div class="grid shipping">
                    ...
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Indirizzo di spedizione e fatturazione', 'nutrisslim-suite'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($billing_address); ?></div>
                    <?php else : ?>
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Indirizzo di spedizione', 'woocommerce'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($shipping_address); ?></div>
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Indirizzo di fatturazione', 'woocommerce'); ?></h4>
                    </div>
                    <div class="col-12"><?php echo wp_kses_post($billing_address); ?></div>
                    <?php endif; ?>
                </div>
            ...
            <div class="col-12 conclusion">
                <p class="text-center">
                    <?php echo __('Grazie per il tuo acquisto. Ti auguriamo una buona giornata!', 'nutrisslim-suite'); ?>
                </p>
                <p class="text-center"><?php echo __('Il tuo Nature\'s Finest', 'nutrisslim-suite'); ?></p>
            </div>
        </div>
    </div>
</section>
...
function thankyouContent() {
	...
	if ( is_checkout() && is_wc_endpoint_url('order-received') ) {
		...
		if ( (in_array($method, ['cod', 'stripe'])) && $enabled && ($is_afterpurchase !== 'yes') ) {
			...
			echo '<h3>' . $first_name . ', ' . __('L\'ordine è stato completato', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Grazie per aver scelto i nostri prodotti.', 'nutrisslim-suite') . '</p>';
            echo '<p>' . __('Numero d\'ordine', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            echo '<p>' . sprintf(__('La conferma dell\'ordine verrà inviata all\'indirizzo e-mail %1$s.', 'text-domain'), '<strong>' . esc_html($order_email) . '</strong>') . '</p>';
			echo '<p><a id="pregled-narucila" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Riepilogo dell\'ordine', 'nutrisslim-suite') . ':</a></p>';
            echo '<div class="timer-after-purchase">';
            echo '<h3>' . __('Offerta speciale', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Sconto aggiuntivo su prodotti selezionati per un periodo di tempo limitato!!!', 'nutrisslim-suite') . '</p>';
            echo '<div class="timer">';
            echo '<div class="minutes timer-circle">1</div>';
            ...
            if ($has_book_file) {
                ...
                foreach ($order->get_items() as $item_id => $item) {
                    ...
                    if ($book_file) {
                        ...
                        echo '<p class="text-center"><a href="' . esc_url($book_file['url']) . '" target="_blank">' . __('Scarica il libro', 'nutrisslim-suite') . '</a></p>';
                        echo '</div>'; // col
                        echo '</div>';
                    }
                }
			...
			echo '<div class="container">';
			echo '<h3>' . sprintf(esc_html__('%1$s, L\'ordine è stato completato', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __('Grazie per aver scelto i nostri prodotti.', 'nutrisslim-suite') . '</strong></p>';

            echo '<p>' . __('Da Nature\'s Finest siamo impegnati a offrire le migliori soluzioni naturali per uno stile di vita sano - e grazie a te possiamo continuare a perseguire questa visione!', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Ti informeremo non appena il pacco sarà pronto.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Numero dell\'ordine', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('La conferma dell\'ordine verrà inviata all\'indirizzo e-mail fornito <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';
            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Mostra offerta aggiuntiva:', 'nutrisslim-suite') . '</a></p>';
            }
...
<?php
            ...
            $ato = sprintf( __( 'Aggiungi all\'ordine', 'nutrisslim-suite') );
            $rfo = sprintf( __( 'Rimuovi dall\'ordine', 'nutrisslim-suite') );            
            ...
            if (in_array($method, ['cod'])) {
                ...
                _e('Completa l\'ordine', 'nutrisslim-suite');
                echo '</a>';    
            } else {
                ...
                _e('Termina l\'ordine', 'nutrisslim-suite');
                echo '</a>';    
            }
            echo '<p class="text-center"><a id="straighttofinal" data-oid="' . $order_id . '" href="#">' . __( 'Non richiedo questa offerta', 'nutrisslim-suite') . '</a></p>';
            echo '</div>';
            ...
            echo '<h3>' . sprintf(__('<span>Ultima occasione</span><br /> Approfitta dello sconto %1$s%%!', 'nutrisslim-suite'), esc_html($discount_percentage)) . '</h3>';
            ...
            echo '<p class="text-center"><a class="gotopregled final" data-oid="' . $order_id . '" href="#">' . __( 'Non accetto questa offerta', 'nutrisslim-suite') . '</a></p>';            
            echo '</div>';		
            echo '</div>';
            echo '</div>';		
			...
			echo '<h3>' . sprintf(esc_html__('%1$s, L\'ordine è stato completato', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __('Grazie per aver scelto i nostri prodotti.', 'nutrisslim-suite') . '</strong></p>';        
            echo '<p>' . __('Da 10 anni, in Nature\'s Finest, siamo impegnati a offrire le migliori soluzioni naturali per una vita sana – e grazie a te possiamo continuare a perseguire questa visione!', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Ti informeremo immediatamente non appena il pacco sarà pronto.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Numero dell\'ordine', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('La conferma dell\'ordine verrà inviata all\'indirizzo e-mail fornito sul sito <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Mostra offerta aggiuntiva:', 'nutrisslim-suite') . '</a></p>';
            }
...
<h4><?php echo __('Visualizza i tuoi ordini', 'woocommerce'); ?></h4>
