...
function add_content_to_footer_on_product_page() {
    if (is_product()) {
        ...
        echo '<h3 id="subscrModalTitle">' . __('Risparmia con l\'abbonamento', 'nutrisslim-suite') . '</h3>';  
        echo '<h4>' . __('Vantaggi dell\'abbonamento', 'nutrisslim-suite') . '</h4>';
        echo '<ul>';
        echo '<li>' . __('Se ti registri oggi, verrà applicato un ulteriore sconto del 5% sul prezzo di acquisto una tantum.', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('Risparmia tempo: ricevi le consegne ogni mese senza dover effettuare ordini.', 'nutrisslim-suite') . '</li>';
        //echo '<li>' . __('<strong>Free gift</strong> with every subscribed shipment, starting from the second shipment.', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('La sottoscrizione può essere annullata in qualsiasi momento dal proprio account nella sezione -> Ordini -> Area abbonamenti oppure semplicemente via e-mail.', 'nutrisslim-suite') . '</li>';
        // echo '<li>' . __('<strong>Free shipping</strong> from the second shipment onwards.', 'nutrisslim-suite') . '</li>';        
        echo '</ul>';
        echo '<p>' . __('Prima della spedizione del pacco, riceverai un\'e-mail di promemoria.', 'nutrisslim-suite') . '</p>';
        echo '</div>';
        echo '</div>';
    }
}
